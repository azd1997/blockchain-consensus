...
const (
	...
	MessageMagicNumber uint32 = 0x789abcde      // 消息魔数，快速确定消息起始位置，校验协议是否匹配
)
...
const (
	...
	MessageType_ReqBlockByHash  MessageType = 101 // 按hash查询
...
type Message struct {
	...
	// 区块索引从1开始，0表示本地还没有区块
	Epoch int64 // 纪元，指当前是基于哪一个区块的创建时间基准 从数量上等同于最新区块index
	...
	Data      [][]byte // 区块/证明/交易 等序列化的数据的列表
	// req
	ReqBlockIndexStart int64
	ReqBlockIndexCount int64    // 正数代表正向获取，负数代表反方向获取
	Hashes             [][]byte // 根据哈希请求
...
func (msg *Message) Encode() ([]byte, error) {
	...
	//err = binary.Write(buf2, binary.BigEndian, MessageMagicNumber, length, m)
	//if err != nil {
	//	return nil, err
	//}
	return m, nil
